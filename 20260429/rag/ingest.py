...
목적
- handover JSON을 읽어서 검색 가능한 문서 단위로 평탄화(flatten)한다.
- ChromaDB에 적재한다.
- 하드코딩을 최소화하기 위해 section / step / job / realtime query를 일반화해서 처리한다.

실행 예시
python ingest.py --json_path ./handover.json --persist_dir ./chroma --collection handover_agent --reset

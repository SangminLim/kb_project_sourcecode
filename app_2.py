...
def render_overview_block(result: Any) -> None:
    ...
    # overview 화면은 JSON 필드 스키마를 기준으로 섹션을 구성한다.
    # 값이 없는 섹션은 표시하지 않아, view_filter로 일부 필드만 내려온 경우에도
    # "등록된 요약 정보가 없습니다" 같은 불필요한 빈 섹션이 노출되지 않는다.
    section_specs = [
        ("summary", "핵심 요약", "🔹"),
        ("content", "상세 설명", "📝"),
    ...
    # 향후 overview JSON 필드가 추가되어도 값이 있으면 기본 섹션으로 표시한다.
    # 필드명 라벨은 최소한의 표시용 alias만 두고, 데이터 자체는 JSON 값을 그대로 사용한다.
    label_map = {
        "owner": "담당자",
        "owner_team": "담당팀",
        "cycle": "처리 주기",
    ...
    if not visible_sections:
        st.info("요청한 업무 개요 항목 정보가 없습니다.")
        return

    with st.container(border=True):
        # 섹션 개수와 무관하게 동일한 렌더링 함수를 사용한다.
        # 1개 필드만 필터링되어 내려오면 1개 섹션만, 전체 overview면 전체 섹션을 표시한다.

...
def validate_dataframe(df: pd.DataFrame) -> None:
    ...
    required_columns: List[str] = validation_rules.get("not_null_columns") or []
    ...
    if null_columns:
        raise ValueError(f"NULL 값이 존재하는 필수 컬럼이 있습니다: {null_columns}")
...
def run(database_url: str, base_ym: str, output_dir: str = "./output") -> Dict[str, Any]:
    if not database_url:
    ...
    df = pd.read_sql(text(sql), engine, params={"base_ym": base_ym})
...
if __name__ == "__main__":
    ...
    run(args.database_url, args.base_ym, args.output_dir)

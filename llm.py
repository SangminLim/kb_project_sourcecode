...
CANONICAL_QUESTION_TEMPLATE_PATH = Path(
    os.getenv(
        "CANONICAL_QUESTION_TEMPLATE_PATH",
        str(CONF_DIR / "canonical_question_templates.json"),
    )
)
REALTIME_INTENT_REGISTRY_PATH = Path(
    os.getenv(
        "REALTIME_INTENT_REGISTRY_PATH",
        str(CONF_DIR / "realtime_intent_registry.json"),
    )
)
...
class AgentWorkflowState(TypedDict, total=False):
    """llm.py 내부 질의 처리 LangGraph state.

    app.py 라우팅 그래프와 별개로, 단일 질의가 답변으로 변환되는 과정을
    prepare -> rewrite -> resolve -> retrieve -> respond 노드로 분리한다.
    값 목록은 고정 업무명이 아니라 기존 설정/registry 기반 결과를 담는 공통 payload다.
    """
    question: str
    chat_history: List[Dict[str, str]]
    top_k: int
